...
void EXAMM::generate_size_count() {
    if (genome_size_log) {
        ...
        // Complete size log headers for islands
    } else {
        size_log_file = NULL;
...
bool EXAMM::insert_genome(RNN_Genome* genome) {
    ...
    // getting island id and insert position of previous global best genome for patience check
    RNN_Genome* previous_best_genome = speciation_strategy->get_global_best_genome();
    int32_t previous_best_generation_id = -1;
    int32_t previous_best_island_id = -1;
    if (previous_best_genome != NULL) {
        previous_best_generation_id = previous_best_genome->get_generation_id();
        previous_best_island_id = previous_best_genome->get_group_id();
    }

    // updates EXAMM's mapping of which genomes have been generated by what
    genome->update_generation_map(generated_from_map);
    int32_t genome_generation_id = genome->get_generation_id();
    int32_t genome_group_id = genome->get_group_id();
    ...
    if (genome_generation_id > patience_trigger_gen && previous_best_generation_id >= 1
        ...
        if(current_patience > patience_rate){
            ...
            Log::info("previous global best island id: %d\n", previous_best_island_id);
            Log::info("previous global best insert position: %d\n", speciation_strategy->get_position_in_population(previous_best_generation_id, previous_best_island_id));
            ...
            if (patience == 1) {
                ...
                speciation_strategy->mutation_rate = 0.3;
                speciation_strategy->intra_island_co_rate = 0.45;
                speciation_strategy->inter_island_co_rate = 0.25;

                Log::info(
                    "Speciation Strategy Rates - Mutation: %.2f, Intra-island Crossover: %.2f, Inter-island Crossover: "
                    "%.2f\n",
                    speciation_strategy->mutation_rate, speciation_strategy->intra_island_co_rate,
                    speciation_strategy->inter_island_co_rate
                );
            } else if (patience == 2) {
                Log::info("Current patience = %d\n", current_patience);
                Log::info("Patience exceeded and pre-convergence found!\n");
                Log::info("Doubling the enable, disable operation rate!\n");
                ...
                Log::info("Edge Rates - Enable: %.2f, Disable: %.2f\n", enable_edge_rate, disable_edge_rate);
                Log::info("Node Rates - Enable: %.2f, Disable: %.2f\n", enable_node_rate, disable_node_rate);
            }
            patience = 0;
        }
    }
